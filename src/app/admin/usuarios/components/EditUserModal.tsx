...
export default function EditUserModal({ isOpen, onClose, user }: { isOpen: boolean, onClose: () => void, user: UserProps }) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    // Add the user id to the form data implicitly (or explicitly if we pass it as a hidden input, let's use hidden input in JSX)
      ...
      <div className="bg-white rounded-2xl shadow-apple-lg w-full max-w-md overflow-hidden relative">
        ...
        <form action={handleSubmit} className="p-6 space-y-4 text-left">
          ...
          <div>
            ...
            <input name="name" type="text" defaultValue={user.name} required className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary outline-none" />
          </div>
          
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Rol en el Sistema</label>
            <select name="role" defaultValue={user.role} className="w-full px-4 py-2.5 rounded-xl border border-gray-200 focus:ring-2 focus:ring-brand-primary/20 focus:border-brand-primary outline-none bg-white">
